...
export class LLRPTypedMessage<
    ...
    static ofType<
        ...
        TD extends AD[N] = AD[N],
        DT extends GetDataType<AD, TD> = GetDataType<AD, TD>,
        >(Def: AD, Name?: N): GetMessageClassType<
            AnyConstructor<LLRPTypedMessage<AD, N, TD, DT>>,
            AnyConstructor<LLRPProxyParameter>,
            AD, N, TD, DT> {

        class LLRPTypedMessage extends this<AD, N, TD, DT> {
            static _def = Def;
        ...
        for (let tRef of td.subTypeRefs) {
            ...
            if (repeat === "0-1" || repeat === "1") {
                LLRPTypedMessage.prototype[`set${name}`] = function <T extends LLRPTypedMessage>(this: T, p: LLRPProxyParameter) {
                    this.setSubParameter(name as any, p);
                    return this;
                }
            } else {
                LLRPTypedMessage.prototype[`add${name}`] = function <T extends LLRPTypedMessage>(this: T, p: LLRPProxyParameter) {
